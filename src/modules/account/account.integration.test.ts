...
describe('AccountAPI (Integration)', () => {
    ...
    beforeAll(async () => {
        ...
        const client = (sdk.account as any).redis;
        if (client?.isReady) {
            // Clear existing test cache to ensure fresh results for the suite
            const keys = await client.keys('rifty:test:*');
            if (keys.length > 0) await client.del(keys);
            console.log(`[Setup] Cleared ${keys.length} test keys from Redis.`);
        }
    });
